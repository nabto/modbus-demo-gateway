...
void uart_initialize_as_unix_uart(struct uart_if* uartIf, uint8_t channel, void* name, uint32_t baudrate, uint8_t databits, uart_parity parity, uart_stopbits stopbits) {
    uartIf->shutdown=&uart_shutdown;
    uartIf->can_read=&uart_can_read;
    uartIf->can_write=&uart_can_write;
    uartIf->read=&uart_read;
    uartIf->read_buffer=&uart_read_buffer;
    uartIf->write=&uart_write;
    uartIf->write_buffer=&uart_write_buffer;
    uartIf->flush_receiver=&uart_flush_receiver;
    uartIf->flush_transmitter=&uart_flush_transmitter;

    uart_initialize(channel, name, baudrate, databits, parity, stopbits);
